...
! -d kallisto || ! -d exp_tables || ! -d bowtie2_rrna ]]
then
  echo "One of the required directories is missing, I will try to create them."
  mkdir bams picard_stats fc_stats tdfs_and_bws RSEM featureCounts FastQC kallisto exp_tables bowtie2_rrna
else
...
cd $WDIR/fastqs
prun_star.sh $WDIR $REFDIR $SPECIES $CPUS
echo 
echo "=================================================================================="
echo
...
cd $WDIR/picard_stats
STRAND1=`grep PCT_CORRECT_STRAND_READS -A 1 *.picard.metrics | awk -F "\t" '{print $23}' | grep -v -P "^$|PCT_CORRECT_STRAND_READS" | awk '{sum+=$1} END {printf "%.2f\n",sum*100/NR}'`
...
STRAND2=`cat *rnastat | awk '{sum+=$16} END {x=sum/NR; if (x<10) \
{print "RF"} else if (x>90) {print "FR"} else if (x>45 && x<55) \
{print "NONE"} else {print "ERROR"}}'`
echo "Strandedness estimates: $STRAND1 (Picard tools), $STRAND2 (featureCounts)."
